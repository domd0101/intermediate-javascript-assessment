...
// CODE HERE...
    ...
    // grammarPolice = function(str){
String.prototype.grammarPolice = function(str){
  //  var fil = str.split('');
  //  var old = [];
  //  var num = [];
  //  for (var i = 0; i < fil.length; i++) {
  //    if(fil[i].search(/[a-z]/i)===0){
  //      old.push(fil[i]);
  //    }
  //  }
  //
  // var newstr = str.replace(/[^a-zA-Z ]/g, "").replace(/\d+/g, '');
  // var slash = '/\/';
  // var n = str.search(slash);
  //
  // str[n] = '\\';
  // str = str.replace(/&/g, '&amp;') /* This MUST be the 1st replacement. */
  ...
  for (var i = 0; i < newstr.length; i++) {
    ...
    newstr[i] = newstr[i].charAt(0).toUpperCase() + newstr[i].toLowerCase().slice(1);
  }
  return newstr.join(' ')
}

// console.log(grammarPolice("`\\ pp ii eldk"));
